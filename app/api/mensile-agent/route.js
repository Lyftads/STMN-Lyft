...
export async function POST(req) {
  ...
  try {
    const { userId, content: reply, usage } = await callBrain({
      ...
      query: lastUserMsg,
      data: context,
      dataLabel: 'DATI MENSILI — usa SOLO questi numeri, mai inventare:',
      dataMax: 70000,
      messages: clean,
      locale: null,
      ...
      temperature: 0.2,
      topP: 0.2,
      guardTail: 'REMINDER: prima di rispondere verifica che OGNI numero e OGNI nome di mese che scrivi sia letteralmente presente nel JSON DATI MENSILI. Se manca, scrivi "Non ho questo dato" invece di inventare.',
    })
