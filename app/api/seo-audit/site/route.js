    ...
    try {
      ...
      const { parsed } = await callBrain({
        skill: {
          id: 'seo',
          json: true,
          systemPrompt: 'Sei un consulente SEO senior e-commerce. Italiano. Dati i problemi SEO ricorrenti su un sito (con n. pagine colpite), restituisci JSON {"recommendations":[{"priority":"alta|media|bassa","title":"...","action":"..."}]}. Max 6, per impatto sul sito intero.',
        },
        query: 'SEO tecnica e on-page e-commerce strategia sito intero',
        messages: [{ role: 'user', content: `Sito: ${new URL(pages[0].url).origin}\nPagine analizzate: ${pages.length}\nScore medio: ${avgScore}\n\nProblemi ricorrenti:\n${top}` }],
        locale: body.locale,
        conversation: false,
        temperature: 0.4,
      })
