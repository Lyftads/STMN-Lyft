...
const STATUS_CFG = {
  pending:  { label: 'In attesa', color: '#fbbf24', bg: 'rgba(251,191,36,0.12)', icon: 'clock' },
  approved: { label: 'Approvata', color: '#2997ff', bg: 'rgba(41,151,255,0.12)', icon: 'check' },
  executed: { label: 'Eseguita',  color: '#30d158', bg: 'rgba(48,209,88,0.12)', icon: 'check-circle' },
  rejected: { label: 'Rifiutata', color: '#86868b', bg: 'rgba(134,134,139,0.12)', icon: 'close' },
  failed:   { label: 'Fallita',   color: '#ff453a', bg: 'rgba(255,69,58,0.12)', icon: 'warning' },
}
    ...
    <div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 18, flexWrap: 'wrap' }}>
        ...
        <div style={{ flex: 1, minWidth: 200 }}>
          <div style={{ fontSize: 20, fontWeight: 900, color: 'var(--text)', letterSpacing: '-0.01em' }}>Coda Azioni</div>
          <div style={{ fontSize: 12.5, color: 'var(--text3)', marginTop: 2 }}>
            Le azioni proposte dai moduli (Budget Advisor, Creative Fatigue…) in attesa di approvazione ed esecuzione.
          </div>
        </div>
        <button onClick={load} disabled={loading} style={ghostBtn}>
          <Icon name="scan" size={13} /> {loading ? 'Aggiorno…' : 'Aggiorna'}
        </button>
      </div>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 16, flexWrap: 'wrap' }}>
        {FILTERS.map(f => {
          const n = f.id === 'all' ? actions.length : (counts[f.id] || 0)
      ...
      {loading && actions.length === 0 ? (
        <div style={{ color: 'var(--text3)', fontSize: 13, padding: 20 }}>Caricamento…</div>
      ) : shown.length === 0 ? (
        <div className="glass-card-static" style={{ padding: 28, textAlign: 'center', color: 'var(--text3)' }}>
          ...
          <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--text)' }}>Nessuna azione {filter !== 'all' ? `"${FILTERS.find(f => f.id === filter)?.label}"` : 'in coda'}</div>
          <div style={{ fontSize: 12.5, marginTop: 4 }}>Apri Budget Advisor o Creative Fatigue e premi “Applica” su un consiglio per accodarlo qui.</div>
        </div>
      ) : (
              ...
              <div key={a.id} className="glass-panel" style={{ borderRadius: 14, padding: 16, borderLeft: `3px solid ${s.color}` }}>
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: 14, flexWrap: 'wrap' }}>
                  ...
                  <div style={{ flex: 1, minWidth: 220 }}>
                    ...
                    <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginTop: 7, fontSize: 11, color: 'var(--text3)' }}>
                      ...
                      {a.source && <span>· da {a.source.replace(/_/g, ' ')}</span>}
                      {a.created_at && <span>· {new Date(a.created_at).toLocaleString('it-IT', { day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit' })}</span>}
                    </div>
                  </div>
                ...
                {me.isAdmin && a.status !== 'executed' && (
                  <div style={{ display: 'flex', gap: 8, marginTop: 12, flexWrap: 'wrap' }}>
                    {a.status === 'pending' && <>
                      <button disabled={busy === a.id} onClick={() => op(a.id, 'approve')} style={btn('#30d158')}><Icon name="check" size={13} /> Approva</button>
                      <button disabled={busy === a.id} onClick={() => op(a.id, 'reject')} style={btnGhost('#ff8095')}><Icon name="close" size={13} /> Rifiuta</button>
                    </>}
                    {a.status === 'approved' && <>
                      <button disabled={busy === a.id} onClick={() => op(a.id, 'execute')} style={btn('#2997ff')}><Icon name="check-circle" size={13} /> Segna come eseguita</button>
                      <button disabled={busy === a.id} onClick={() => op(a.id, 'reopen')} style={btnGhost()}>Riapri</button>
                    </>}
                    {a.status === 'rejected' && (
                      <button disabled={busy === a.id} onClick={() => op(a.id, 'reopen')} style={btnGhost()}>Riapri</button>
                    )}
                    <button disabled={busy === a.id} onClick={() => remove(a.id)} style={btnGhost('#86868b')}><Icon name="trash" size={13} /> Elimina</button>
                  </div>
                )}
              </div>
            )
