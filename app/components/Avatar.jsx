...
export default function Avatar({ name, url, size = 32, online }) {
  ...
  const inner = url
    ...
    : <div style={{ width: size, height: size, borderRadius: '50%', background: 'linear-gradient(135deg,#7b5bff,#5b8bff)', color: 'var(--text)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: Math.round(size * 0.4), fontWeight: 700, fontFamily: 'Barlow' }}>{initials}</div>
  return (
