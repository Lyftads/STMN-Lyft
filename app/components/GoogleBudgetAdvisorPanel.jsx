...
// chiavi i18n ba.*/aq.* (etichette platform-agnostic). Accoda in Coda Azioni.
...
function ApplyButton({ qstate, onClick }) {
  ...
  return (
    <button onClick={onClick} disabled={qstate === 'busy'} title={t('aq.applyTitle')} style={{
      ...
      background: 'rgba(234,179,8,0.16)', border: '1px solid rgba(234,179,8,0.4)',
...
const ACT = {
  scala: { color: 'var(--green)', bg: 'rgba(48,209,88,0.14)' },
  mantieni: { color: 'var(--text2)', bg: 'var(--glass2)' },
            ...
            {re.freed > 0 && (
              <div className="glass-card-static" style={{ padding: '14px 16px', borderRadius: 12, borderLeft: '3px solid var(--accent)', marginBottom: 16, display: 'flex', alignItems: 'center', gap: 14, flexWrap: 'wrap' }}>
                ...
                <ApplyButton qstate={queued.realloc} onClick={() => enqueue('realloc', {
                  channel: 'google', source: 'google_budget_advisor', type: 'shift_budget',
                  ...
                  <div key={i} className="glass-card-static" style={{ padding: 12, borderRadius: 14, display: 'flex', alignItems: 'center', gap: 14, borderLeft: `3px solid ${a.color}` }}>
                    ...
                    {c.action !== 'mantieni' && (() => {
                      ...
                      return <ApplyButton qstate={queued[key]} onClick={() => enqueue(key, {
                        channel: 'google', source: 'google_budget_advisor',
              ...
              <div className="glass-card-static reveal-zoom" style={{ marginTop: 22, padding: 18, borderRadius: 16 }}>
                ...
                <ResponsiveContainer width="100%" height={250}>
                  <BarChart data={chartData} margin={{ top: 6, right: 8, left: -10, bottom: 0 }}>
                    <defs>
                      <linearGradient id="gbaAtt" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#eab308" stopOpacity={0.95} /><stop offset="100%" stopColor="#eab308" stopOpacity={0.35} /></linearGradient>
                      <linearGradient id="gbaCon" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#30d158" stopOpacity={0.95} /><stop offset="100%" stopColor="#30d158" stopOpacity={0.35} /></linearGradient>
                    </defs>
                    ...
                    <Bar dataKey="attuale" name={t('ba.chart.current')} fill="url(#gbaAtt)" radius={[5, 5, 0, 0]} animationDuration={1400} animationEasing="ease-out" />
                    <Bar dataKey="consigliata" name={t('ba.chart.suggested')} fill="url(#gbaCon)" radius={[5, 5, 0, 0]} animationDuration={1400} animationEasing="ease-out" />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            )}
