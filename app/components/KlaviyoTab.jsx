...
function Card({ title, value, badge, color = '#8b5cf6', hint }) {
  return (
    <div className="glass-card" style={cardStyle} title={hint || undefined}>
      ...
      {/* Sotto al numero, non solo nel suggerimento: se un dato puo' essere
          scambiato per un altro, dirlo va scritto dove si guarda. */}
      {hint && <div style={{ fontSize: 10.5, color: '#776a86', marginTop: 6, lineHeight: 1.4 }}>{hint}</div>}
    </div>
  )
}
                ...
                <tr key={c.id || i}
                  ...
                  <td style={{ padding: '10px 16px', color: '#f7f2ff', fontWeight: 700 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
                      <span>{c.name}</span>
                      <Icon name="eye" size={12} style={{ color: '#776a86' }} />
                      {c.status === 'Draft' && (
...
function CampaignPreview({ campaign, tr, onClose }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/klaviyo/campaign?id=${encodeURIComponent(campaign.id)}`, { cache: 'no-store', signal: AbortSignal.timeout(30000) })
      .then(r => r.json())
  ...
  return (
    <div onClick={onClose} style={{
      position: 'fixed', inset: 0, zIndex: 1000, background: 'rgba(0,0,0,0.72)', backdropFilter: 'blur(6px)',
      display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '3vh 16px',
    }}>
      <div onClick={e => e.stopPropagation()} style={{
        width: 'min(760px, 100%)', maxHeight: '94vh', display: 'flex', flexDirection: 'column',
        background: '#15111c', border: '1px solid var(--border)', borderRadius: 16, overflow: 'hidden',
      }}>
        <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'flex-start', gap: 12 }}>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontSize: 15, fontWeight: 900, color: '#f7f2ff', letterSpacing: '-0.01em' }}>{campaign.name}</div>
            <div style={{ fontSize: 11.5, color: '#9b90aa', marginTop: 3 }}>
              {campaign.status}{campaign.sendTime ? ` · ${new Date(campaign.sendTime).toLocaleString('it-IT')}` : ''}
            </div>
          </div>
          <button onClick={onClose} aria-label={tr('klaviyo.close', null, 'Chiudi')} style={{ background: 'none', border: 'none', color: '#9b90aa', cursor: 'pointer', fontSize: 22, lineHeight: 1, padding: 0 }}>×</button>
        </div>

        <div style={{ overflowY: 'auto', flex: 1 }}>
          {!d && !err && <div style={{ padding: 40, textAlign: 'center', color: '#9b90aa', fontSize: 13 }}>{tr('klaviyo.loadingPreview', null, 'Carico l’anteprima…')}</div>}
            ...
            <>
              <div style={{ padding: '16px 20px', display: 'flex', flexDirection: 'column', gap: 12, borderBottom: '1px solid var(--border)' }}>
                ...
                <Riga label={tr('klaviyo.subject', null, 'Oggetto')}>
                  <span style={{ color: '#f7f2ff', fontWeight: 700, fontSize: 14 }}>{d.message.subject || '—'}</span>
                </Riga>
                <Riga label={tr('klaviyo.previewText', null, 'Testo di anteprima')}>
                  <span style={{ color: '#c9c2d4', fontSize: 13 }}>{d.message.previewText || '—'}</span>
                </Riga>
                <Riga label={tr('klaviyo.sender', null, 'Mittente')}>
                  <span style={{ color: '#c9c2d4', fontSize: 13 }}>
              ...
              {d.body.html ? (
                ...
                <iframe title={d.message.subject || campaign.name} srcDoc={d.body.html}
                  sandbox="allow-popups allow-popups-to-escape-sandbox"
                  style={{ width: '100%', height: '58vh', border: 'none', background: '#fff', display: 'block' }} />
              ) : d.body.text ? (
                <pre style={{ margin: 0, padding: 20, whiteSpace: 'pre-wrap', color: '#c9c2d4', fontSize: 12.5, lineHeight: 1.6 }}>{d.body.text}</pre>
              ) : (
...
function Riga({ label, children }) {
  return (
    <div style={{ display: 'flex', gap: 12, alignItems: 'baseline', flexWrap: 'wrap' }}>
      <span style={{ fontSize: 10.5, color: '#776a86', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '.07em', minWidth: 118 }}>{label}</span>
