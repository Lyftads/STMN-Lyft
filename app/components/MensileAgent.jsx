...
const SUGGESTIONS = [
  'Confronta il mese selezionato con il precedente',
  'Qual è stato il mese migliore e perché?',
  'Il MER come si sta muovendo mese su mese?',
  'CAC in trend: peggiora o migliora?',
]
...
export default function MensileAgent({ data, selectedMonth, previousMonth, preset }) {
  ...
  const send = async (text) => {
    ...
    let metrics = null
    try {
      const presetParam = preset || `month_${selectedMonth}`
        ...
        <div style={{ borderTop: '1px solid var(--border)', padding: '14px 18px 18px' }}>
          ...
          <form onSubmit={e => { e.preventDefault(); send() }} style={{ display: 'flex', gap: 8 }}>
            <input
              ...
              placeholder="Chiedi qualcosa sui dati mensili…"
              disabled={loading}
              style={{
                flex: 1,
                background: 'var(--glass)',
