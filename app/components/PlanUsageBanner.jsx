...
export default function PlanUsageBanner({ onGoSettings }) {
  ...
  return (
    <div style={{
      ...
      <span style={{ color: 'var(--attenzione)', display: 'inline-flex' }}><Icon name="chart-line" size={20} /></span>
      <div style={{ flex: 1, minWidth: 240, fontSize: 13.5, lineHeight: 1.5 }}>
        ...
        {' '}{t('plan.upgradeTo', null, 'Passa al piano')} <b style={{ color: 'var(--text)' }}>{data.recommended.label}</b> {data.recommended.price !== 'su misura' ? `(${data.recommended.price}/mese)` : `(${t('plan.customPriceLc', null, 'su misura')})`} {t('plan.stayCompliant', null, 'per restare in regola.')}
      </div>
      <button onClick={() => (onGoSettings ? onGoSettings() : null)} style={{
        background: 'var(--btn-primario)', border: 'none', borderRadius: 9,
        padding: '9px 16px', color: 'var(--btn-primario-testo)', fontSize: 13, fontWeight: 800, cursor: 'pointer', fontFamily: 'Barlow', whiteSpace: 'nowrap',
      }}>{t('plan.upgradeBtnTo', { plan: data.recommended.label }, `Passa a ${data.recommended.label}`)}</button>
      <button onClick={dismiss} title={t('plan.hideThisMonth', null, 'Hide for this month')} style={{ background: 'none', border: 'none', color: 'var(--text3)', cursor: 'pointer', fontSize: 18, lineHeight: 1 }}>×</button>
    </div>
  )
}
