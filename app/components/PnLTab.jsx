...
export default function PnLTab({ data = [] }) {
  ...
  const rows = useMemo(() => {
    ...
    return series.map(s => {
      ...
      const adsMeta = adsByChannel[s.month]?.meta ?? 0
      ...
      return { ...s, fatturato, net, cogs, grossMargin, ads, adsMeta, adsGoogle, fee, packaging, shipCost, contrib, fixed: fixedTotal, ebit, ebitPct }
    })
  }, [state.series, state.feesByMonth, state.gatewayMix, state.cogsByMonth, state.cogsRatio, cogsRatio, adByMonth, adsByChannel, fattByMonth, cfg, fixedTotal])
  ...
  const annual = useMemo(() => {
    ...
    return {
      ...
      fatturato: sum('fatturato'),
      ads: sum('ads'), adsMeta: sum('adsMeta'), adsGoogle: sum('adsGoogle'),
  ...
  const lines = [
    ...
    { label: t('pnl.lineAdsMeta', null, 'di cui Meta'), key: 'adsMeta', neg: true, sub: true },
    { label: t('pnl.lineAdsGoogle', null, 'di cui Google'), key: 'adsGoogle', neg: true, sub: true },
...
function GraficoPnL({ asc, monthLabelL, t }) {
  ...
  const serie = [
    ...
    { key: 'costi', label: t('pnl.seriesCosts', null, 'Costi'), color: '#ff5a7a' },
    { key: 'ebit', label: 'EBIT', color: '#30d158' },
  ]
  ...
  return (
    <>
      <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 14, flexWrap: 'wrap' }}>
        <div style={{ fontWeight: 700, fontSize: 13, opacity: 0.85, letterSpacing: '-0.01em' }}>
      ...
      <div style={{ width: '100%', height: 300 }}>
        <ResponsiveContainer>
          <ComposedChart data={dati} margin={{ top: 10, right: 6, left: -10, bottom: 0 }}>
            ...
            {accesa('costi') && (
              <Line type="monotone" dataKey="costi" name={serie[1].label} stroke="#ff5a7a" strokeWidth={1.4}
            ...
            {accesa('ebit') && (
              <Line type="monotone" dataKey="ebit" name={serie[2].label} stroke="#30d158" strokeWidth={2}
...
function Etichetta({ active, payload, label, serie }) {
  ...
  return (
    <div style={{
      background: 'rgba(8,8,15,0.94)', backdropFilter: 'blur(16px)',
      border: '1px solid rgba(255,255,255,0.10)', borderRadius: 10,
      padding: '10px 12px', fontSize: 12, minWidth: 140,
      ...
      {payload.map(p => {
        ...
        return (
          <div key={p.dataKey} style={{ display: 'flex', alignItems: 'center', gap: 9, marginTop: 3 }}>
            <span style={{ width: 2, height: 13, borderRadius: 2, background: col, flexShrink: 0 }} />
  ...
  return (
    <div style={{ marginTop: 14, fontSize: 12, lineHeight: 1.6 }}>
      <button onClick={() => setAperta(v => !v)} style={{
        ...
        color: INC_BLU, fontStyle: 'italic', fontSize: 12, fontWeight: 600, fontFamily: 'inherit',
      }}>
...
function MoM({ cur, prev, lowerBetter = false, titolo }) {
  ...
  return <div title={titolo} style={{ fontSize: 9.5, fontWeight: 600, color: col, marginTop: 2, whiteSpace: 'nowrap', cursor: 'help' }}>{up ? '▲' : '▼'} {Math.abs(pct).toFixed(0)}% · {up ? '+' : '−'}{eur(Math.abs(d))}</div>
}
...
const thInc = { padding: '10px 10px 10px 4px', borderBottom: '1px solid var(--border)', fontSize: 9.5, opacity: 0.55, textAlign: 'right', whiteSpace: 'nowrap', color: INC_BLU, fontStyle: 'italic', fontWeight: 600, minWidth: 54 }
const tdInc = { padding: '9px 10px 9px 4px', textAlign: 'right', whiteSpace: 'nowrap', color: INC_BLU, fontStyle: 'italic', fontWeight: 500, fontSize: 11, fontVariantNumeric: 'tabular-nums', cursor: 'pointer' }
...
const tdYoy = { padding: '9px 8px', textAlign: 'right', whiteSpace: 'nowrap', color: 'var(--text3)', fontWeight: 500, fontSize: 12, fontVariantNumeric: 'tabular-nums', cursor: 'pointer', ...STACCO_MESE }
