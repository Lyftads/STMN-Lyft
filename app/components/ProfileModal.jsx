...
const PANEL = { background: 'var(--surface)', border: '1px solid var(--border2)', borderRadius: 12, padding: 20 }
const input = { background: 'var(--surface2)', border: '1px solid var(--border2)', borderRadius: 8, padding: '10px 12px', color: 'var(--text)', fontSize: 14, fontFamily: 'Barlow', width: '100%', outline: 'none' }
const btn = { background: 'var(--btn-primario)', border: 'none', borderRadius: 8, padding: '10px 16px', color: 'var(--btn-primario-testo)', fontSize: 14, fontWeight: 700, cursor: 'pointer', fontFamily: 'Barlow' }
    ...
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(4px)', zIndex: 1100, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '4vh 16px', fontFamily: 'Barlow' }}>
      <div onClick={e => e.stopPropagation()} style={{ ...PANEL, width: 'min(420px, 100%)' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 16 }}>
          ...
          <label style={{ ...btn, background: 'transparent', border: '1px solid var(--border2)', cursor: 'pointer' }}>
            {t('profile.changePhoto', null, 'Change photo')}
            <input type="file" hidden accept=".png,.jpg,.jpeg,.webp,.gif" onChange={pick} />
          </label>
        </div>

        <label style={{ fontSize: 12, color: 'var(--text3)' }}>{t('profile.fullName', null, 'Full name')}</label>
        ...
        <div style={{ fontSize: 12, color: 'var(--text3)', marginBottom: 16 }}>{profile?.email}</div>

        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10 }}>
          <button onClick={onClose} style={{ ...btn, background: 'transparent', border: '1px solid var(--border2)' }}>{t('common.cancel', null, 'Cancel')}</button>
