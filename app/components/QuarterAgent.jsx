...
const SUGGESTIONS = [
  'Confronta il trimestre selezionato con il precedente',
  'Qual è stato il trimestre migliore e perché?',
  'Come si muove il MER trimestre su trimestre?',
  'CAC trimestrale: peggiora o migliora?',
]
...
export default function QuarterAgent({ quarters, selectedQuarter, previousQuarter, preset }) {
  const t = useT()
  ...
  const send = async (text) => {
    ...
    let metrics = null
    try {
      const presetParam = preset || `quarter_${selectedQuarter}`
    ...
    try {
      const r = await fetch('/api/quarter-agent', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ locale: getClientLocale(), messages: next, quarters, selectedQuarter, previousQuarter, metrics }),
      })
    ...
    <>
      {!open && (
        <button
          ...
          aria-label={`${t('common.open', null, 'Apri')} Quarter Agent`}
          title="Quarter Agent"
          style={{
            position: 'fixed',
            bottom: 28,
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <div style={{ minWidth: 0 }}>
              <div style={{ fontSize: 15, fontWeight: 700, color: 'var(--text)' }}>Quarter Agent</div>
              <div style={{ fontSize: 11, color: 'var(--text3)', marginTop: 2, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                Confronto trimestrale · Stagionalità · Strategia
              </div>
            </div>
          </div>
          <button
        ...
        <div style={{ borderTop: '1px solid var(--border)', padding: '14px 18px 18px' }}>
          ...
          <form onSubmit={e => { e.preventDefault(); send() }} style={{ display: 'flex', gap: 8 }}>
            <input
              ...
              placeholder="Chiedi qualcosa sui dati trimestrali…"
              disabled={loading}
              style={{
                flex: 1,
                background: 'var(--glass)',
...
function AvatarSvg() {
  return (
    <svg width="60" height="60" viewBox="0 0 60 60" fill="none">
      <defs>
        <radialGradient id="qBgGlow" cx="0.5" cy="0.3" r="0.7">
          <stop offset="0%" stopColor="#dbeafe" stopOpacity="0.20" />
          <stop offset="100%" stopColor="#1e293b" stopOpacity="0" />
        </radialGradient>
        <linearGradient id="qSuit" x1="0.5" y1="0" x2="0.5" y2="1">
          <stop offset="0%" stopColor="#0f172a" />
          <stop offset="100%" stopColor="#020617" />
        </linearGradient>
        <linearGradient id="qShirt" x1="0.5" y1="0" x2="0.5" y2="1">
          <stop offset="0%" stopColor="#f8fafc" />
          <stop offset="100%" stopColor="#cbd5e1" />
        </linearGradient>
        <linearGradient id="qSkin" x1="0.5" y1="0" x2="0.5" y2="1">
          <stop offset="0%" stopColor="#fbd9b8" />
          <stop offset="100%" stopColor="#d4a373" />
        </linearGradient>
        <linearGradient id="qSkinShade" x1="0.5" y1="0" x2="0.5" y2="1">
          <stop offset="0%" stopColor="#000" stopOpacity="0" />
          <stop offset="100%" stopColor="#000" stopOpacity="0.18" />
        </linearGradient>
        <linearGradient id="qHair" x1="0.5" y1="0" x2="0.5" y2="1">
          <stop offset="0%" stopColor="#3b2f24" />
          <stop offset="50%" stopColor="#1f1813" />
          <stop offset="100%" stopColor="#0d0a08" />
        </linearGradient>
        <linearGradient id="qHairHi" x1="0.5" y1="0" x2="0.5" y2="1">
          <stop offset="0%" stopColor="#6b5a47" stopOpacity="0.55" />
          <stop offset="100%" stopColor="#3b2f24" stopOpacity="0" />
        </linearGradient>
      </defs>

      <circle cx="30" cy="30" r="30" fill="url(#qBgGlow)" />
      <path d="M5 60c0-10 10-15 25-15s25 5 25 15v0H5z" fill="url(#qSuit)" />
      <path d="M22 45l8 8 8-8-3-3h-10z" fill="url(#qShirt)" />
      <path d="M22 45l-2 4 5-1 5 5 5-5 5 1-2-4-8 5z" fill="url(#qSuit)" />
      <path d="M25 39h10v5l-2 3h-6l-2-3z" fill="url(#qSkin)" />
      <rect x="25" y="44" width="10" height="3" fill="url(#qSkinShade)" />
      <ellipse cx="30" cy="29" rx="11" ry="13" fill="url(#qSkin)" />
      <ellipse cx="30" cy="34" rx="10" ry="6" fill="url(#qSkinShade)" opacity="0.6" />
      <path d="M19 22c0-7 5-12 11-12s11 5 11 12c0 1-.1 2-.3 3-.6-1.5-2-3.5-4-4.5-1.5 1-3.7 1.8-6.7 1.8s-5.2-.8-6.7-1.8c-2 1-3.4 3-4 4.5-.2-1-.3-2-.3-3z" fill="url(#qHair)" />
      <path d="M22 19c1.5-3 4.5-5 8-5s6.5 2 8 5c-2-1.5-4.5-2.5-8-2.5s-6 1-8 2.5z" fill="url(#qHairHi)" />
