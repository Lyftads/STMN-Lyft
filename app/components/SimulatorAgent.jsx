...
const SUGGESTIONS = [
  'Quale scenario conviene scalare e perché?',
  'Dammi un piano cash flow per scalare lo scenario migliore',
  'Strategia per migliorare il margine netto e ridurre il break-even ROAS',
  'Cosa fare per spingere il ROAS dello scenario Aggressivo',
  'Forecast ottimistico e pessimistico a 12 mesi',
  'Quali leve (AOV, COGS, ROAS) muovere prima per dare profittabilità',
]
          ...
          {messages.length === 0 && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
              <div style={{
                ...
                {timeGreeting()} Marino. Cosa vuoi simulare oggi?
              </div>
            ...
            <div key={i} style={{ display: 'flex', justifyContent: m.role === 'user' ? 'flex-end' : 'flex-start' }}>
              <div style={{
                ...
                background: m.role === 'user'
                  ? 'linear-gradient(135deg, #1e3a8a, #2997ff)'
        ...
        <div style={{ borderTop: '1px solid var(--border)', padding: '14px 18px 18px' }}>
          ...
          <form onSubmit={e => { e.preventDefault(); send() }} style={{ display: 'flex', gap: 8 }}>
            <input
              ...
              placeholder="Chiedi setup, cash flow, scaling, leve…"
              disabled={loading}
              style={{
                flex: 1,
                background: 'rgba(255,255,255,0.04)',
...
function AvatarSvg() {
  return (
    <svg width="60" height="60" viewBox="0 0 60 60" fill="none">
      <defs>
        <radialGradient id="simBgGlow" cx="0.5" cy="0.3" r="0.7">
          <stop offset="0%" stopColor="#dbeafe" stopOpacity="0.20" />
          <stop offset="100%" stopColor="#1e293b" stopOpacity="0" />
        </radialGradient>
        <linearGradient id="simSuit" x1="0.5" y1="0" x2="0.5" y2="1">
          <stop offset="0%" stopColor="#1e3a8a" />
          <stop offset="100%" stopColor="#0c1e4f" />
        </linearGradient>
        <linearGradient id="simShirt" x1="0.5" y1="0" x2="0.5" y2="1">
          <stop offset="0%" stopColor="#f8fafc" />
          <stop offset="100%" stopColor="#cbd5e1" />
        </linearGradient>
        <linearGradient id="simSkin" x1="0.5" y1="0" x2="0.5" y2="1">
          <stop offset="0%" stopColor="#fbd9b8" />
          <stop offset="100%" stopColor="#d4a373" />
        </linearGradient>
        <linearGradient id="simSkinShade" x1="0.5" y1="0" x2="0.5" y2="1">
          <stop offset="0%" stopColor="#000" stopOpacity="0" />
          <stop offset="100%" stopColor="#000" stopOpacity="0.18" />
        </linearGradient>
        <linearGradient id="simHair" x1="0.5" y1="0" x2="0.5" y2="1">
      ...
      <circle cx="30" cy="30" r="30" fill="url(#simBgGlow)" />
      <path d="M5 60c0-10 10-15 25-15s25 5 25 15v0H5z" fill="url(#simSuit)" />
      <path d="M22 45l8 8 8-8-3-3h-10z" fill="url(#simShirt)" />
      <path d="M22 45l-2 4 5-1 5 5 5-5 5 1-2-4-8 5z" fill="url(#simSuit)" />
      <path d="M25 39h10v5l-2 3h-6l-2-3z" fill="url(#simSkin)" />
      <rect x="25" y="44" width="10" height="3" fill="url(#simSkinShade)" />
      <ellipse cx="30" cy="29" rx="11" ry="13" fill="url(#simSkin)" />
      <ellipse cx="30" cy="34" rx="10" ry="6" fill="url(#simSkinShade)" opacity="0.6" />
      <path d="M19 22c0-7 5-12 11-12s11 5 11 12c0 1-.1 2-.3 3-.6-1.5-2-3.5-4-4.5-1.5 1-3.7 1.8-6.7 1.8s-5.2-.8-6.7-1.8c-2 1-3.4 3-4 4.5-.2-1-.3-2-.3-3z" fill="url(#simHair)" />
