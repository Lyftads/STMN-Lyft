...
const SOLUTIONS = [
  ...
  { key: 'ads', items: ['Meta Detail & KPI', 'Google KPI & Detail', 'Google Products (PMax)', 'Google Lighthouse & Budget', 'Creative & Budget Advisor', 'Creative Fatigue', 'Incrementalità & MMM'] },
...
//  Page
// ─────────────────────────────────────────────────────────────
// La lingua arriva dalla ROTTA (/welcome = it, /en, /es, /fr, /de). Cosi' l'HTML
// servito al crawler e' gia' nella lingua giusta: prima veniva scelta solo dopo
// l'idratazione, quindi Google vedeva sempre e solo l'italiano.
export default function LandingPage({ initialLang = null }) {
  ...
  useEffect(() => {
    ...
    // Rotta con lingua esplicita (/en, /es…): e' gia' quella giusta, non toccarla.
  ...
  // Cambiare lingua cambia anche l'URL: /welcome per l'italiano, /en /es /fr /de
  // per le altre. Serve perche' la lingua sia condivisibile con un link e perche'
  // ogni versione abbia una pagina propria da indicizzare.
  const chooseLang = (code) => {
    langChosen.current = true
    setLang(code)
    try {
      const path = code === 'it' ? '/welcome' : `/${code}`
