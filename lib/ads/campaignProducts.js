...
export async function deriveMetaProducts({ token, accounts, products }) {
  if (!token || !accounts?.length) return new Map()
  const key = `metaprod|${accounts.join(',')}|${products.length}`
  const hit = __cache.get(key)
  if (hit && hit.exp > Date.now()) return hit.data
  const maps = buildShopifyMaps(products)
  const data = await fetchMetaCampaignProducts(token, accounts, maps).catch(() => new Map())
