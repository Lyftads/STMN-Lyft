...
import { getRange } from '../metaRange'
...
const LTV_CFG = { freq: 1.69, life: 1.57, margin: 100 } // fallback neutro: senza costi margine=100 (regola progetto)
...
export function periodKPIs(d, period) {
  ...
  // META TAB-ESATTO per periodo da _metaPeriods (/api/meta-detail); per mesi da
  // metaAds.monthly; riserva: serie weekly nel range.
  const mp2 = (d._metaPeriods || {})[period]
  const metaMo = key => { const mo = (ma.monthly || []).find(m => String(m.month || m.date || '').slice(0, 7) === key); return mo ? Number(mo.spend) : null }
  let spend = mp2?.spend
  if (spend == null && (period === 'this_month' || period === 'last_month')) spend = metaMo(from.slice(0, 7))
  if (spend == null) spend = sum(mw, w => w.spend)
...
export function buildBrief(d) {
  ...
  const sLine = (key, lbl) => { const s = shop(key); return `  ${lbl}: ${s ? `${eur(s.f)}, ${r1(s.o)} ordini, AOV ${eur(s.o ? s.f / s.o : 0)}` : 'in aggiornamento'}` }

  const MP = d._metaPeriods || {}
  ...
  // spesa Meta TAB-ESATTA per periodo: meta-detail (giorni/7g/14g/mese scorso), monthly (mese corr.), weekly (settimane/30g)
  const metaSpend = period => {
    if (MP[period]?.spend != null) return MP[period].spend
    if (period === 'this_month') return monthSpend(thisMonthK)
    if (period === 'last_month') return monthSpend(lastMonthK)
    if (period === 'last_30d') return m30
    if (period === 'this_week') return mw(thisMon)?.spend ?? null
    if (period === 'last_week') return mw(lastMon)?.spend ?? null
    return null
  }
  const mLine = (val, lbl) => `  ${lbl}: ${val != null ? eur(val) : 'n/d'}`

  // Spesa GOOGLE ADS per periodo, sommata dalla serie giornaliera (_googleDaily)
  // con le STESSE date di Meta (lib/metaRange) → i due canali sono confrontabili.
  const gDaily = Array.isArray(d._googleDaily) ? d._googleDaily : []
  const gSum = period => {
    if (!gDaily.length) return null
    const { since, until } = getRange(period)
    const rows = gDaily.filter(x => { const k = String(x.date || '').slice(0, 10); return k >= since && k <= until })
    if (!rows.length) return null
    const sumF = f => rows.reduce((s, x) => s + (Number(f(x)) || 0), 0)
    const spend = sumF(x => x.spend), value = sumF(x => x.convValue), conv = sumF(x => x.conversions)
    return { spend, roas: spend ? value / spend : null, conv }
  }
  const gaLine = (period, lbl) => {
    const g = gSum(period)
    return `  ${lbl}: ${g ? `${eur(g.spend)}${g.roas ? `, ROAS ${g.roas.toFixed(1)}` : ''}, ${r1(g.conv)} conversioni` : 'n/d'}`
  }
  const googleBlock = gDaily.length ? `\n\nSPESA pubblicitaria GOOGLE ADS, per periodo (esatta):
${gaLine('today', 'Oggi')}
${gaLine('yesterday', 'Ieri')}
${gaLine('last_7d', 'Ultimi 7 giorni')}
${gaLine('this_week', 'Questa settimana')}
${gaLine('last_week', 'Scorsa settimana')}
${gaLine('this_month', 'Questo mese')}
${gaLine('last_month', 'Scorso mese')}
${gaLine('last_30d', 'Ultimi 30 giorni')}` : ''
...
VENDITE Shopify (fatturato netto · ordini · AOV), per periodo:
${sLine('today', 'Oggi')}
${sLine('yesterday', 'Ieri')}
${sLine('last_7d', 'Ultimi 7 giorni')}
...
SPESA pubblicitaria Meta, per periodo (esatta):
${mLine(metaSpend('today'), 'Oggi')}
${mLine(metaSpend('yesterday'), 'Ieri')}
${mLine(metaSpend('last_7d'), 'Ultimi 7 giorni')}
${mLine(metaSpend('this_week'), 'Questa settimana')}
${mLine(metaSpend('last_week'), 'Scorsa settimana')}
${mLine(metaSpend('this_month'), 'Questo mese')}
${mLine(metaSpend('last_month'), 'Scorso mese')}
${mLine(metaSpend('last_30d'), 'Ultimi 30 giorni (circa)')}${googleBlock}
