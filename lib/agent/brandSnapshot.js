...
export function periodKPIs(d, period) {
  ...
  const pm = P[period]
  const tab = (d._metricsPeriods || {})[period]
  ...
  if (revenue == null && tab && tab.fatturato != null) { revenue = tab.fatturato; orders = tab.orders }
  ...
  const spend = sum(mw, w => w.spend), impressions = sum(mw, w => w.impressions), clicks = sum(mw, w => w.linkClicks)
  const freq = mw.length ? sum(mw, w => w.frequency) / mw.length : null
  ...
  const roas_meta = Number(d.metaDetail?.summary?.roas) || (spend ? revenue / spend : null)
...
export function buildBrief(d) {
  ...
  const shop = key => {
    ...
    const tab = (d._metricsPeriods || {})[key]; if (tab && tab.fatturato != null) return { f: tab.fatturato, o: tab.orders } // tab-esatto (/api/metrics)
