...
export function periodKPIs(d, period) {
  ...
  // money: preferisci _periods (esatto, netto resi)
  const pm = P[period]
  let revenue = pm?.fatturato != null ? pm.fatturato : sum(sw, w => w.fatturato)
  let orders = pm?.orders != null ? pm.orders : sum(sw, w => w.ordini)
  const nc = sum(sw, w => w.nc), rc = sum(sw, w => w.rc)
