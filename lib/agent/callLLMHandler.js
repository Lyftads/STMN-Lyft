...
- get_google_campaigns(periodo): campagne Google Ads con spesa, ROAS, conversioni. Funziona per QUALSIASI periodo, oggi e ieri compresi.
...
CANALI SEPARATI: nel riepilogo Meta e Google Ads sono DUE blocchi distinti. La spesa Meta non è quella di Google e viceversa: non usare mai il numero di un canale per l'altro, e se ti chiedono la spesa totale sommali dicendo che è la somma dei due. Se la riga di un periodo è "n/d", NON dire che il dato non esiste: chiama lo strumento del canale (get_google_campaigns per Google) con quel periodo.`
  ...
  try {
    ...
    if (Array.isArray(msg.tool_calls) && msg.tool_calls.length) {
      ...
      const rawToken = body.extra_body?.lyft_ws || body.elevenlabs_extra_body?.lyft_ws || body.custom_llm_extra_body?.lyft_ws || body.lyft_ws || null
      const toolCtx = {
        origin, workspaceId: ownerUser, snapshot: liveData,
        // Token firmato PRIMA di tutto (anche per l'owner): il percorso cron
        // risolve le credenziali dalle ENV, dove il refresh token Google è
        // quello revocato a luglio → in call Google risultava assente mentre
        // nel DB c'è il token valido.
        ...(rawToken && verifyCallToken(rawToken) ? { callToken: rawToken } : {}),
        ...(!rawToken && isOwnerCall ? { cronSecret: process.env.CRON_SECRET || '' } : {}),
      }
