...
function buildBrief(d) {
  ...
  // Settimane PRECISE come la tab Weekly: chiave = lunedì (UTC). this_week=lunedì
  // corrente, last_week=lunedì scorso. Numeri ESATTI dalla serie weekly.
  ...
  const findW = key => (Array.isArray(sh.weekly) ? sh.weekly : []).find(w => String(w.date || '').slice(0, 10) === key)
  const tw = findW(thisMon), lw = findW(lastMon)
  const wline = (w, lbl) => w ? `${lbl}: ${r1(w.fatturato)} euro, ${r1(w.ordini)} ordini (ESATTI)` : `${lbl}: dato non ancora disponibile (ShopifyQL in aggiornamento sugli ultimi giorni)`
  const weeksBlock = `\nSETTIMANE (numeri ESATTI dalla tab Weekly — usali tali e quali):\n${wline(tw, `QUESTA settimana (da lun ${thisMon} a oggi)`)}\n${wline(lw, `SCORSA settimana (lun ${lastMon} → dom)`)}`
