...
// Brief dati COMPATTO (poche centinaia di caratteri) → prompt piccolo = risposta veloce.
function buildBrief(d) {
  ...
  const rev = wk.reduce((s, w) => s + (Number(w.fatturato) || 0), 0)
  const ord = wk.reduce((s, w) => s + (Number(w.ordini) || 0), 0)
  const maWk = Array.isArray(d.metaAds?.weekly) ? d.metaAds.weekly.slice(-4) : []
  const spend = maWk.reduce((s, w) => s + (Number(w.spend) || 0), 0)
  const roas = Number(d.metaDetail?.summary?.roas) || 0
  const prods = (sh.topProducts || []).slice(0, 5).map(p => `${p.label || p.product} €${r1(p.revenue)}`).join(', ')
  const cre = (Array.isArray(d.creatives) ? d.creatives : []).slice(0, 5).map(c => `"${c.name}" ROAS ${c.roas ?? '-'}`).join('; ')
  return `DATI REALI del brand (ultime ~4 settimane, usa SOLO questi numeri):
Fatturato €${r1(rev)} · Ordini ${ord} · AOV €${ord ? r1(rev / ord) : 0} · Spesa Ads €${r1(spend)} · MER ${spend ? (rev / spend).toFixed(2) : '-'}x · ROAS Meta ${roas ? roas.toFixed(2) : '-'}x
Email: open ${kl.openRate ?? '-'}% · click ${kl.clickRate ?? '-'}% · Sessioni ${ga.sessions ?? '-'}
...
export async function handleCallLLM(req) {
  ...
  // Dati + memoria in PARALLELO (riduce la latenza).
  const ownerUser = process.env.LYFT_OWNER_USER_ID
  const [liveData, mems] = await Promise.all([
    getLiveData(origin),
