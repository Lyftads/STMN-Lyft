...
export async function handleCallLLM(req) {
  ...
  // ── GATING call di gruppo: evita che TUTTI gli agent rispondano insieme. ──
  // Risponde solo l'agente chiamato per nome; se nessun nome è citato, solo la
  // lead (Chiara/ceo). Gli altri restano in silenzio (stream vuoto → niente voce).
  if (groupMode) {
    const NAME_BY_ID = { ceo: 'chiara', cfo: 'marco', cmo: 'luigi', ads: 'sofia', seo: 'davide', cro: 'giulia', data: 'alessandro', creative: 'valentina' }
    const text = ' ' + lastUser.toLowerCase() + ' '
    const mentioned = Object.entries(NAME_BY_ID).filter(([, nm]) => new RegExp(`[^\\p{L}]${nm}[^\\p{L}]`, 'u').test(text)).map(([id]) => id)
    const addressed = mentioned.includes(agentId)
    const shouldSpeak = addressed || (mentioned.length === 0 && agentId === 'ceo')
    if (!shouldSpeak) {
