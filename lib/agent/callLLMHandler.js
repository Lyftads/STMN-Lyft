...
function buildBrief(d) {
  ...
  const pick = (stat, key) => (stat && (stat.orders || stat.fatturato)) ? { fatturato: stat.fatturato, ordini: stat.orders } : (() => { const w = findW(key); return w ? { fatturato: w.fatturato, ordini: w.ordini } : null })()
