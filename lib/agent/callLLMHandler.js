...
function buildBrief(d) {
  ...
  const findMW = key => (Array.isArray(d.metaAds?.weekly) ? d.metaAds.weekly : []).find(w => String(w.date || '').slice(0, 10) === key)
  const twM = findMW(thisMon), lwM = findMW(lastMon)
  const wline = (lbl, s, m) => {
    const p = []
    p.push(s ? `fatturato ${r1(s.fatturato)} euro, ${r1(s.ordini)} ordini` : 'vendite in aggiornamento')
    if (m && Number(m.spend)) p.push(`spesa ads Meta ${r1(m.spend)} euro`)
    return `${lbl}: ${p.join(' · ')}`
  }
  const weeksBlock = `\nSETTIMANE (numeri ESATTI per settimana — usa QUESTI per domande su questa/scorsa settimana, NON i totali dei 30 giorni):\n${wline(`QUESTA settimana (da lun ${thisMon} a oggi)`, tw, twM)}\n${wline(`SCORSA settimana (lun ${lastMon} → dom)`, lw, lwM)}`
...
Advertising (Meta, totale ultimi ~30 giorni): Spesa ~${k(spend)} euro · ROAS ${roas ? roas.toFixed(1) : '-'} · MER ${spend ? (rev / spend).toFixed(1) : '-'}
