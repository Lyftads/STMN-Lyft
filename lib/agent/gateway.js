...
import { resolveTier, chatWithFallback } from './router'
import { headers as nextHeaders } from 'next/headers'
import { ALL_TOOLS, FAST_TOOLS, executeToolLive } from './tools'

// ── Tool LIVE automatici (stile Sidekick, per TUTTO il SaaS) ───────────────
// Se callBrain gira dentro una richiesta (cookie di sessione o job con
// x-internal-cron), i tool live si attivano DA SOLI per ogni consumer:
// chat per-tab, insight, azioni proattive, note/to-do, standup, report, call.
// Chat conversazionali → ALL_TOOLS; flussi generativi JSON → FAST_TOOLS
// (senza i tool lenti). Fuori da una richiesta (script/test) → nessun tool.
function autoToolCtx() {
  try {
    const h = nextHeaders()
    const host = h.get('host')
    if (!host) return null
    const cookie = h.get('cookie') || ''
    const cronSecret = h.get('x-internal-cron') || ''
    if (!cookie && !cronSecret) return null
    const proto = h.get('x-forwarded-proto') || 'https'
    return { origin: `${proto}://${host}`, cookie, cronSecret }
  } catch { return null }
}

// Provider di default per gli override `model` espliciti (OpenAI). Per le call
// senza model, il provider+modello li sceglie il router in base al tier della
// skill (docs/AI_PROVIDER.md). Il fallback cross-provider arriva in Fase 3.
...
const GREETING_RE = /^(ciao|hey|hola|salve|buongiorno|buonasera|buon pomeriggio|hi|hello|hallo|bonjour|good (morning|evening))[\s!.,…]*$/i
...
export async function callBrain({
  ...
  liveTools = true,    // false → disattiva l'auto-attivazione dei tool live
} = {}) {
  ...
  // Tool live automatici: attivi per ogni consumer dentro una richiesta,
  // salvo che il chiamante passi già i suoi tools/onToolCall o opti fuori.
  if (liveTools && !tools && !onToolCall) {
    const toolCtx = autoToolCtx()
    if (toolCtx) {
      tools = conversation ? ALL_TOOLS : FAST_TOOLS
  ...
  try {
    const ctx = await buildAgentContext({ agentId: skill.id, query: recallQuery, conversationLength: cleanHistory.length, includeConversation: conversation , greetingOnly: GREETING_RE.test(String(query || '').trim()) })
    userId = ctx.userId
  ...
  const sys = []
  // Prefisso STABILE per primo (persona/skill, spesso enorme): massimizza il
  // prompt caching del provider. Il contextBlock (memorie/recall) cambia a
  // OGNI messaggio: se stesse prima, invaliderebbe la cache di tutto il resto.
  ...
  // Risoluzione provider+modello: `model` esplicito o `skill.model` → provider
  // OpenAI (escape hatch). Altrimenti il router sceglie in base al tier della
  // skill (default 'smart' = comportamento attuale se l'env non è settata).
  let activeProvider = provider
  let activeModel = model || skill.model
  if (!activeModel) {
    const r = resolveTier(skill.tier || 'smart')
  ...
  const callOnce = async (msgs, withTools) => {
    const attempt = () => chatWithFallback(activeProvider, {
      ...
      // signal creato a OGNI tentativo (non condiviso col retry)
      signal: AbortSignal.timeout(55000),
    })
    try {
      return await attempt()
    } catch (e) {
      // 429/5xx/timeout senza provider di fallback configurato: un retry con
      // backoff breve evita l'errore secco in faccia all'utente.
      const st = e?.status
  ...
  while (tools && onToolCall && rounds < 3) {
    ...
    // Esecuzione in PARALLELO: con 2-3 tool per round i tempi non si sommano.
    const toolResults = await Promise.all(m.tool_calls.map(async (tc) => {
      ...
      return { role: 'tool', tool_call_id: tc.id, content: (() => { const _s = JSON.stringify(result); return _s.length <= 8000 ? _s : _s.slice(0, 8000) + '… [RISULTATO TRONCATO: chiedi un periodo o un filtro più ristretto per il resto]' })() }
    }))
  ...
  let parsed = useJson ? safeParse(content) : null

  // JSON malformato → UN retry deterministico. Prima il chiamante riceveva
  // parsed:null e degradava in silenzio (la finalize di una call perdeva
  // sintesi, decisioni e task senza che nessuno se ne accorgesse).
  if (useJson && parsed === null && content) {
    try {
      const retryMsgs = [...msgs, { role: 'assistant', content },
        { role: 'user', content: 'La risposta precedente non era JSON valido. Rispondi di nuovo con SOLO il JSON richiesto, senza testo attorno.' }]
      const res2 = await callOnce(retryMsgs, false)
      const c2 = res2.content || ''
      const p2 = safeParse(c2)
      if (p2 !== null) { content = c2; parsed = p2 }
      else console.log('[brain] json retry fallito', { skill: skill?.id })
    } catch (e) { console.log('[brain] json retry threw:', e?.message) }
  }
  ...
  return {
    ...
    content,
    parsed,
    parsedError: useJson && parsed === null ? 'json_invalid' : null,
