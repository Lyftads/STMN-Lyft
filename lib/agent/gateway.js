...
export async function callBrain({
  ...
  try {
    const ctx = await buildAgentContext({ agentId: skill.id, query: recallQuery, conversationLength: cleanHistory.length })
    userId = ctx.userId
  ...
  if (data != null) sys.push({ role: 'system', content: `DATI LIVE:\n${safeJson(data)}` })

  const body = {
    model: model || skill.model || DEFAULT_MODEL,
    temperature: temperature ?? skill.temperature ?? 0.3,
    messages: [...sys, ...cleanHistory, ...(lang ? [lang] : [])],
