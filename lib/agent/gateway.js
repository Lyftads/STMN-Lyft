...
export async function callBrain({
  ...
  const callOnce = async (msgs, withTools) => {
    const r = await fetch(OPENAI_URL, {
      ...
      body: JSON.stringify({ ...baseBody, messages: msgs, ...(withTools && tools ? { tools, tool_choice: 'auto' } : {}) }),
      signal: AbortSignal.timeout(55000),
    })
    if (!r.ok) { const t = await r.text(); const err = new Error(`OpenAI ${r.status}: ${t.slice(0, 400)}`); err.status = r.status; throw err }
    return r.json()
  }
  ...
  let j = await callOnce(msgs, !!(tools && onToolCall))
  let rounds = 0
  while (tools && onToolCall && rounds < 3) {
    ...
    rounds++
    j = await callOnce(msgs, rounds < 2)
  }
