...
function buildPrompt(p) {
  return `Sei "${p.agentName}", consulente di fiducia del founder del brand descritto nel CONTESTO BRAND, iper-specializzato sull'analisi ${p.adjective}.
...
export async function handlePeriodAgent(req, kind) {
  ...
  try {
    ...
    if (userId && lastUserMsg && reply) {
      waitUntil(Promise.resolve(persistTurnMemory({ agentId: p.id, userId, userMessage: lastUserMsg, assistantMessage: reply })).catch(() => {}))
    }
    if (userId && context) {
      waitUntil(Promise.resolve(persistDataMemory({ agentId: p.id, userId, data: context })).catch(() => {}))
    }
