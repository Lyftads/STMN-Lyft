...
export async function chatWithFallback(provider, params) {
  try {
    ...
    try {
      // Signal NUOVO: riusando quello del primario (già abortito o quasi
      // scaduto) il fallback moriva all'istante e non scattava mai.
      const out = await fallback.chatComplete({
        ...params,
        model: fallback.defaultModel() || params.model,
