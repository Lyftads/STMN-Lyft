...
export async function ordersInRange(start, end, maxPages = 40) {
  ...
  const gql = `query($q:String!,$cursor:String){orders(first:100,query:$q,after:$cursor,sortKey:CREATED_AT){edges{cursor node{currentTotalPriceSet{shopMoney{amount}} totalRefundedSet{shopMoney{amount}} customer{numberOfOrders}}}pageInfo{hasNextPage}}}`
  const q = `created_at:>=${start}T00:00:00${offS} created_at:<=${end}T23:59:59${offE} financial_status:paid`
  let cursor = null, pages = 0, orders = 0, fatturato = 0, resi = 0, nc = 0, rc = 0
  try {
    while (pages < maxPages) {
      ...
      for (const e of edges) {
        ...
        if (net > 0.01) { orders += 1; if (Number(e.node.customer?.numberOfOrders || 0) <= 1) nc += 1; else rc += 1 }
      }
      pages++
    ...
    return { orders, fatturato: Math.round(fatturato), resi: Math.round(resi), nc, rc }
  } catch { return null }
}
...
export async function periodStats() {
  ...
  const ranges = {
    ...
    last_7d: [addDays(today, -6), today], last_14d: [addDays(today, -13), today],
