...
export async function periodStats() {
  ...
  const ranges = {
    ...
    // Come le tab (/api/metrics getPresetRange) e il report di Shopify: "ultimi N giorni" = da
    // oggi−N a oggi. Cosi' il numero detto a voce e' quello che si legge nella tab.
    last_7d: [piuGiorni(today, -7), today], last_14d: [piuGiorni(today, -14), today],
    last_30d: [piuGiorni(today, -30), today],
  }
