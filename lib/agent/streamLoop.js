...
async function oaiStreamCall(payload, { onDelta } = {}) {
  ...
  while (true) {
    ...
    for (const line of lines) {
      ...
      if (j.error) throw new Error(j.error?.message || 'Errore stream provider')
...
export async function runToolLoopStream({ body, messages, tools = null, onToolCall = null, maxRounds = 3, onDelta, onToolStart = null, onToolEnd = null }) {
  let msgs = messages
  let rounds = 0
  while (true) {
    ...
    if (useTools && out.toolCalls.length) {
      ...
      // Stato visibile all'utente: 3-10s di silenzio erano il vero problema
      // di latenza percepita, non la durata in sé.
      try { onToolStart?.(out.toolCalls.map(tc => tc.function?.name).filter(Boolean)) } catch {}
