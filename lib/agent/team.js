...
Parli SEMPRE in prima persona come ${agent.name} (${agent.role}), come una persona vera — niente preamboli da AI, niente "come assistente". Rispondi nella lingua dell'utente, in modo conciso e umano.`
}

// Roster compatto (senza systemPrompt) per il frontend / per far conoscere gli
// agenti tra loro.
export function teamRoster() {
  return TEAM.map(({ id, name, role, gender, color, emoji, tagline }) => ({ id, name, role, gender, color, emoji, tagline }))
}
