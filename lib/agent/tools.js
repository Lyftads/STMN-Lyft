...
async function liveResult(name, args, ctx) {
  ...
  if (name === 'get_google_campaigns') {
    ...
    return { data: { rows, totals: d?.summary || null }, jit: `Google Ads live. ${JIT_NUM} ROAS Google = valore conv. ÷ spesa; le campagne PMAX non hanno breakdown keyword.${note ? ' ' + note : ''}` }
  }
