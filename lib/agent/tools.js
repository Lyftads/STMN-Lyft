...
async function liveResult(name, args, ctx) {
  if (name === 'get_kpis') {
    ...
    return { data: d, jit: `KPI live del periodo ${preset}. ${JIT_NUM} ROAS/MER già calcolati: non ricalcolarli.${note ? ' ' + note : ''}` }
  }
  if (name === 'list_creatives' || name === 'list_adsets') {
    ...
    return { data: { rows, totals: d?.totals || null }, jit: `Meta Ads live (level ${level}). ${JIT_NUM} Cita i NOMI esatti di campagne/adset/creative.${note ? ' ' + note : ''}` }
  }
