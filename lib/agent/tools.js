...
export const LIVE_TOOLS = [
  ...
  { type: 'function', function: { name: 'get_finance', description: 'CASSA REALE del brand (open banking): saldo conti bancari, entrate/uscite ultimi 90 giorni, uscite per categoria, proiezione di cassa a 30/60/90 giorni. Usalo per valutare se un investimento (es. aumento budget ads) è sostenibile.', parameters: { type: 'object', properties: {} } } },
]
...
async function liveResult(name, args, ctx) {
  ...
  if (name === 'get_finance') {
    const d = await ifetch(ctx, '/api/cassa')
    if (!d?.configured || !(d.balances || []).length) {
      return { data: { connected: false }, jit: 'Cassa non collegata: suggerisci di collegare la banca nella tab Cassa (2 minuti, sola lettura). NON stimare la liquidità.' }
    }
    return {
      data: { totalBalance: d.totalBalance, inflow90: d.inflow90, outflow90: d.outflow90, projection: d.projection, byCategory: capArr(d.byCategory, 8), balances: capArr(d.balances, 6) },
      jit: `Cassa REALE da open banking. ${JIT_NUM} La proiezione 30/60/90 è basata sulle medie storiche (basedOnDays): presentala come stima, non certezza. Quando consigli di aumentare un budget, verifica che la proiezione a 30-60g resti positiva con margine; altrimenti sconsiglia o proponi un aumento più piccolo.`,
    }
  }
