...
// `giornaliero: true` = vista legata al GIORNO (oggi contro ieri, briefing del mattino): una
// risposta salvata in un altro giorno del negozio non vale piu', nemmeno come «vecchia ma buona».
// Senza, al primo accesso della giornata si vedeva il confronto del giorno prima («ieri, domenica
// 20» quando ieri era lunedi' 21) — Marino, 22 set 2026.
export async function swrSnapshot(req, { tab, ttlMs = DEFAULT_TTL, compute, cacheable = true, giornaliero = false }) {
  ...
  if (!force) {
    ...
    const altroGiorno = giornaliero && snap && giornoNegozio(Date.now() - snap.ageMs) !== giornoNegozio(Date.now())
    const valido = snap && snap.ageMs <= MAX_STALE_MS && !periodoChiusoDopo(url, snap) && !altroGiorno
