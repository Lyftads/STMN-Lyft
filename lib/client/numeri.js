...
// La lingua dei numeri segue quella dell'interfaccia (21 set 2026). Su AV c'e' solo l'italiano;
// nel SaaS in inglese «€49.391» si leggeva «49 virgola 391». La imposta I18nProvider mentre
// disegna, cosi' i componenti figli la trovano gia' giusta nello stesso giro. In italiano non
// cambia niente: punto per le migliaia, virgola per i decimali.
const LOCALI = { it: 'it-IT', en: 'en-GB', es: 'es-ES', fr: 'fr-FR', de: 'de-DE' }
let locale = 'it-IT'
export function linguaNumeri(l) { locale = LOCALI[String(l || 'it').slice(0, 2)] || 'it-IT' }
export function localeNumeri() { return locale }
// Il separatore dei decimali della lingua dell'interfaccia: ',' in italiano, '.' in inglese. Per
// chi scrive ancora `toFixed(d).replace('.', ',')`: stessi arrotondamenti e stesso meno di prima,
// e in italiano esce identico.
export function sepDecimali() { return (1.5).toLocaleString(locale).charAt(1) }
