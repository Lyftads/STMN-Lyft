...
export async function fetchAllCustomersBulk(store, token, deadline = Date.now() + 120000) {
  // NB: niente `displayName` → richiede l'approvazione "Protected Customer Data"
  // specifica di Shopify (alcune app collegate non ce l'hanno → query rifiutata).
  // Il nome si ricava dall'email lato parsing (toBuyers).
  const q = `{ customers { edges { node { id email numberOfOrders amountSpent { amount currencyCode } createdAt lastOrder { createdAt } } } } }`
