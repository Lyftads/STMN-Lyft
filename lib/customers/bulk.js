...
async function currentBulk(store, token) {
  const d = await gql(store, token, `{ currentBulkOperation(type: QUERY) { id status errorCode objectCount url query } }`)
...
// Due query sono "la stessa" se differiscono solo per spazi e a capo.
const stessaQuery = (a, b) => String(a || '').replace(/\s+/g, '') === String(b || '').replace(/\s+/g, '')

async function runBulkQuery(store, token, innerQuery, deadline) {
  // Shopify fa girare UNA operazione bulk per negozio. Se ne gira gia' una con la STESSA query
  // (es. Clienti e LTV & Coorti insieme) si aspetta quella; se e' un'ALTRA (es. gli ordini del
  // backfill) il suo file non e' il nostro: si aspetta che finisca e poi si parte. Prima si
  // prendeva comunque il file dell'operazione in corso, qualunque cosa contenesse.
  for (;;) {
    try { await startBulk(store, token, innerQuery); break }
    catch (e) {
      if (!e.userError || !/already in progress/i.test(e.message || '')) throw e
      const cur = await currentBulk(store, token)
      if (cur && stessaQuery(cur.query, innerQuery)) break
      if (Date.now() > deadline) throw new Error('Bulk occupata da un\'altra operazione')
      await sleep(3000)
    }
  }
