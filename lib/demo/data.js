...
function demoMetrics() {
  ...
  for (let i = 5; i >= 0; i--) {
    ...
    const o = round(f / 72)
    months.push({ month: d.toISOString().slice(0, 7), fatturato: f, ordini: o, nc: round(o * 0.6), rc: round(o * 0.4), uniqueSessions: round(o * 46), resi: round(o * 0.02) })
  }
  ...
  return {
    ...
    shopifyRange: (() => { const l = shopifyWeekly.slice(-4); return { revenue: sum(l, 'fatturato'), orders: sum(l, 'ordini'), nc: sum(l, 'nc'), rc: sum(l, 'rc'), sessions: sum(l, 'uniqueSessions'), resi: sum(l, 'resi') } })(),
    metaRange: (() => { const l = metaWeekly.slice(-4); return { spend: sum(l, 'spend'), impressions: sum(l, 'impressions'), clicks: sum(l, 'linkClicks') } })(),
    shopifyPrevRange: (() => { const l = shopifyWeekly.slice(-8, -4); return { revenue: sum(l, 'fatturato'), orders: sum(l, 'ordini'), nc: sum(l, 'nc'), rc: sum(l, 'rc'), sessions: sum(l, 'uniqueSessions'), resi: sum(l, 'resi') } })(),
