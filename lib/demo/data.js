...
export function demoData(path, search, method = 'GET') {
  ...
  if (p === '/api/gsc') {
    ...
    return {
      ...
      branded: { branded: { clicks: round(totClicks * 0.28), impressions: round(totImpr * 0.2) }, nonBranded: { clicks: round(totClicks * 0.72), impressions: round(totImpr * 0.8) } },
  ...
  // ── Creative Lab / SEO history (azione-driven: stato vuoto pulito) ──
