...
function demoActions(search) {
  ...
  const all = [
    ...
    { id: 'da8', channel: 'instagram', type: 'create_post', target_name: 'Reel bestseller', summary: 'Pubblica post Instagram: «POV: hai trovato il prodotto…»', source: 'social_studio', status: 'pending', payload: { hook: 'POV: hai trovato il prodotto che cercavi 👀', scheduled_for: ymd(now + 2 * DAY) }, created_at: iso(now - 1 * 3600e3) },
    { id: 'da9', channel: 'tiktok', type: 'create_post', target_name: 'Trend audio', summary: 'Pubblica post TikTok: «3 motivi per provarlo oggi»', source: 'social_studio', status: 'approved', payload: { hook: '3 motivi per provarlo oggi', scheduled_for: ymd(now + 5 * DAY) }, created_at: iso(now - 2 * 3600e3) },
  ]
...
export function demoData(path, search, method = 'GET') {
  ...
  if (p === '/api/social/draft-post') return { ok: true, draft: { platform: 'instagram', hook: 'POV: hai trovato il prodotto che cercavi da mesi 👀', caption: 'Il nostro bestseller è tornato disponibile — e questa volta in edizione limitata.\n\nMateriali premium, spedizione in 24/48h, reso facile. Migliaia di clienti soddisfatti non possono sbagliarsi.\n\nTocca il link in bio prima che finisca. 🔥', hashtags: ['#acmestore', '#bestseller', '#dtc', '#madewithlove', '#shopnow', '#newdrop'], cta: 'Scopri la collezione → link in bio', format: 'Reel' } }
