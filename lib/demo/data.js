...
function demoGeoLift() {
  ...
  return {
    ok: true, days, metricNote: 'revenue', metric: 'revenue',
...
export function demoData(path, search, method = 'GET') {
  ...
  if (p === '/api/cassa') {
    if (method !== 'GET') return { error: 'Nella demo la banca è già collegata — provala sul tuo account reale.' }
    if (search && search.get && search.get('action') === 'institutions') {
      return { configured: true, institutions: [
        { id: 'demo-intesa', name: 'Intesa Sanpaolo', logo: null, country: 'IT', psuTypes: ['business'] },
        { id: 'demo-unicredit', name: 'UniCredit', logo: null, country: 'IT', psuTypes: ['business'] },
        { id: 'demo-bpm', name: 'Banco BPM', logo: null, country: 'IT', psuTypes: ['business'] },
      ] }
    }
    return demoCassa()
  }
