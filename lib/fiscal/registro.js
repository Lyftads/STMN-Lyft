...
export async function calcolaRegistro(mese) {
  ...
  for (const q of righeQL) {
    ...
    // L'aliquota e' quella in vigore IL GIORNO della vendita, non quella di
    // oggi: ricostruendo mesi passati la differenza finisce in dichiarazione.
    const aliquota = perimetro === 'SENZA_PAESE' ? null : aliquotaOrdinaria(iso, giorno)
    if (aliquota == null && perimetro !== 'SENZA_PAESE') senzaAliquota++

    const lordo = soldi(q.total_sales)
    if (daFatturazione) lordoDaFatturazione = r2(lordoDaFatturazione + lordo)
    const { imponibile, iva } = scorpora(lordo, aliquota)
    righe.push({
      giorno,
      iso,
