...
// Dal NOME del paese alla sigla. Shopify manda il nome per esteso ('Italy'),
// i centroidi sono indicizzati per sigla: senza questa conversione il globo non
// sapeva dove mettere gli ordini e li lasciava fuori in silenzio.
const ALIAS_PAESE = { 'united states': 'US', 'usa': 'US', 'united kingdom': 'GB', 'uk': 'GB', 'czech republic': 'CZ', 'russia': 'RU', 'south korea': 'KR', 'turkey': 'TR', 'türkiye': 'TR', 'uae': 'AE', 'united arab emirates': 'AE', 'the netherlands': 'NL' }
const PER_NOME = new Map(Object.entries(COUNTRY_CENTROIDS).map(([k, v]) => [String(v.name).toLowerCase(), k]))
export function siglaDelPaese(nome) {
  const n = String(nome || '').trim().toLowerCase()
  return ALIAS_PAESE[n] || PER_NOME.get(n) || null
}

// Un punto con una posizione nota (il centro di una regione) e un piccolo
// scarto per citta', cosi' due citta' della stessa regione non si sovrappongono.
export function attornoA(lat, lng, city, ampiezza = 0.9) {
  const { dLat, dLng } = cityJitter(city)
  return { lat: lat + (dLat / 6) * ampiezza, lng: lng + (dLng / 6) * ampiezza }
}
