...
export function designGeoLift(regions, opts = {}) {
  ...
  const topK = Math.min(opts.topK || 16, usable.length)
  const top = [...usable].sort((a, b) => b.total - a.total).slice(0, topK)
  ...
  // SE del lift ≈ cvRatio / sqrt(D); MDE = (z_alpha/2 + z_power) * SE, α=.05 power=.8 → 2.8
  const Z = 2.8
