...
const BASE_FACTOR = { meta: 0.70, google: 0.55 }
// ROAS riportato di ripiego, se la piattaforma non passa il valore conversione.
const DEFAULT_REPORTED_ROAS = { meta: 3.0, google: 4.0 }
...
export function fitIncrementality(rows, channels, opts = {}) {
  ...
  for (const p of pre) {
    ...
    out.channels.push({
      ...
      attributedRevenue: p.reportedMissing ? null : p.reported,
      ...
      roasReported: (!p.reportedMissing && p.spendTot > 0) ? p.reported / p.spendTot : 0,
