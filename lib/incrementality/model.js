...
export function fitIncrementality(rows, channels, opts = {}) {
  ...
  for (const c of channels) {
    ...
    const k = percentile(ad, 0.8) // half-saturation ~ all'80° percentile → punto operativo informativo
  ...
  // Ridge α proporzionale alla SCALA delle feature (diagonale media di XᵀX sui
  // regressori penalizzati), non alla varianza di y: altrimenti la penalità è
  // enorme e schiaccia tutti i coefficienti a 0 (R²→0, ROAS→0).
  const ncol = X[0].length
  const meanDiag = (() => { let s = 0; for (let j = 1; j < ncol; j++) { let c = 0; for (let i = 0; i < n; i++) c += X[i][j] * X[i][j]; s += c } return s / (ncol - 1) })()
  const alpha = (opts.ridge ?? 0.15) * (meanDiag || 1)
  let beta = ridge(X, y, alpha)
