...
// Saturazione: il "ginocchio" della curva (mezza-saturazione k) è ancorato a un
// percentile della spesa adstockata del canale. Prior per canale: Meta satura
// PRIMA (k a percentile più basso → saturazione più alta a parità di spesa);
// la Search satura più tardi.
const SAT_KNEE = { meta: 0.60, google: 0.85 }
...
export function fitIncrementality(rows, channels, opts = {}) {
  ...
  for (const c of channels) {
    ...
    const k = percentile(ad, SAT_KNEE[c] ?? 0.75) // ginocchio per canale
    cfg[c] = { lambda, k, spend, ad, active: ad.map(v => hill(v, k)) }
    for (let i = 0; i < n; i++) totAd[i] += ad[i]
  }
  const spendCorrR2 = Math.max(0, Math.min(1, corr(totAd, y) ** 2))
  ...
  // Il modello è PRIOR-BASED: la stima NON viene dalla correlazione spesa↔ricavi
  // (che con spesa always-on è ~0 per costruzione: spesa quasi costante, ricavi
  // dominati dalla stagionalità). Usarla come affidabilità mostrava SEMPRE
  // "bassa" anche con dati ottimi. Contano invece:
  //  · copertura: quanti giorni di storia alimentano la stima
  //  · ricavi riportati REALI vs stimati (conv. value importato o no)
  //  · coerenza attribuzione: Σ riportato non deve superare il fatturato
  //  · segnale nella spesa: se la spesa varia, la correlazione aggiunge fiducia;
  //    se è piatta è NON-informativa (neutrale, non un malus)
  const coverageScore = Math.min(1, n / 90)
  const totSpendAll = pre.reduce((s, p) => s + p.spendTot, 0)
  const realSpend = pre.reduce((s, p) => s + (p.reportedMissing ? 0 : p.spendTot), 0)
  const reportedScore = totSpendAll > 0 ? realSpend / totSpendAll : 0
  const realReported = pre.reduce((s, p) => s + (p.reportedMissing ? 0 : p.reported), 0)
  const repRatio = totalRevenue > 0 && realReported > 0 ? realReported / totalRevenue : null
  const sanityScore = repRatio == null ? 0.6 : repRatio <= 0.8 ? 1 : repRatio <= 1.2 ? 0.8 : repRatio <= 2 ? 0.5 : 0.25
  const spendDaily = rows.map(r => channels.reduce((s, c) => s + Math.max(0, Number(r.channels?.[c]) || 0), 0))
  const mSpend = spendDaily.reduce((s, v) => s + v, 0) / n
  const sdSpend = Math.sqrt(spendDaily.reduce((s, v) => s + (v - mSpend) ** 2, 0) / n)
  const spendCv = mSpend > 0 ? sdSpend / mSpend : 0
  const signalScore = spendCv < 0.2 ? 0.6 : Math.max(0.2, Math.min(1, spendCorrR2 / 0.25))
  const confidence = Math.max(0, Math.min(1,
    0.35 * coverageScore + 0.30 * reportedScore + 0.20 * sanityScore + 0.15 * signalScore))
  const confidenceParts = {
    days: n, coverageScore, reportedScore, sanityScore, signalScore,
    spendCv, repRatio, spendCorrR2,
  }
  ...
  const out = { ok: true, days: n, r2: spendCorrR2, confidence, confidenceParts, totalRevenue, baselineRevenue: baselineDay * n, channels: [], daily: [] }
  const incrDaily = {}
  ...
  for (const p of pre) {
    ...
    out.channels.push({
      ...
      halfSatSpend: cf.k * (1 - cf.lambda), // spesa/giorno al ginocchio della curva
