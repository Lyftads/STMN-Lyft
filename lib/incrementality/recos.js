...
export function contributionRecos(data, h) {
  ...
  // Affidabilità: usa lo score composito (v7); fallback legacy su r2 per cache vecchie.
  const lowConf = data?.confidence != null ? data.confidence < 0.45 : (data?.r2 != null && data.r2 < 0.35)
  if (lowConf) out.push({ level: 'info', text: t('reco.lowConf2', null, 'Limited reliability: the total is solid but the channel split is uncertain — validate with a geo-lift before big budget shifts.') })
