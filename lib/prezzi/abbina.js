...
export function chiaviSku(sku) {
  ...
  // La taglia si toglie: lettere (M, XL, UNI) sempre, numeri da calzatura (33-49) solo se restano
  // almeno modello + colore. Senza la seconda regola «FLPNTSELE12-LGREY-37» dava colore «37» e
  // modello «FLPNTSELE12LGREY»: due taglie dello stesso colore non si riconoscevano come lo stesso
  // colore. Un «30» che e' il colore (PIQUADRO 01802401-30-UNI) resta al suo posto, perche' li'
  // dopo la taglia restano solo due pezzi.
  const taglia = (x) => { const u = String(x).toUpperCase(); if (TAGLIE.has(u)) return true; const n = Number(x); return /^\d{2}$/.test(x) && n >= 33 && n <= 49 && pezzi.length > 2 }
  while (pezzi.length > 1 && taglia(pezzi[pezzi.length - 1])) pezzi.pop()
