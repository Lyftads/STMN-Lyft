...
export async function quadro(ws) {
  ...
  for (const a of (nostri?.articoli || [])) {
    ...
    let minimo = null
    if (offerte.length) {
      // Il confronto vero e' sullo STESSO colore: se c'e' un abbinamento esatto vale quello, anche se
      // un altro colore dello stesso modello costa meno (spesso e' un colore in saldo). Senza esatti
      // resta il modello.
    ...
    // margine: il prezzo e' IVA compresa (22%), il costo no
    const margineA = (prezzo) => (a.costo != null && prezzo > 0 ? +(prezzo / IVA - a.costo).toFixed(2) : null)
    const margine = margineA(a.prezzo)
    righe.push({ ...a, offerte, minimo, scarto: minimo ? pct(minimo.prezzo) : null,
      margine, marginePct: margine != null && a.prezzo > 0 ? +((margine / (a.prezzo / IVA)) * 100).toFixed(1) : null,
