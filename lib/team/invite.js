...
export async function sendChatInviteEmail({ to, origin, password, intro }) {
  ...
  const link = `${origin || process.env.NEXT_PUBLIC_APP_URL || 'https://stmn-lyft.vercel.app'}/chat`
  const html = `<div style="font-family:Arial,sans-serif;max-width:520px;margin:auto">
    <h2 style="color:#5b8bff">Sei stato invitato nella chat di LyftAI</h2>
    <p>${intro || 'Sei stato invitato a partecipare alla chat del team.'}</p>
  ...
  try {
    const res = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({ from, to: [to], reply_to: process.env.REPLY_TO || 'info@lyftads.agency', subject: 'Invito alla chat di LyftAI', html }),
    })
