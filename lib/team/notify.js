...
export async function sendEmail({ to, subject, html, attachments }) {
  ...
  const replyTo = process.env.REPLY_TO || 'info@lyftads.agency'
...
// Inserisce una notifica in-app per il destinatario (team_members.id).
export async function addNotification({ workspaceId, recipientId, type, title, body, taskId, tab = 'tasks' }) {
  if (!recipientId) return
  try {
    const admin = getAdminSupabase()
    if (!admin) return
    await admin.from('notifications').insert({
      workspace_id: workspaceId, recipient_id: recipientId, type,
      title, body: body || null, task_id: taskId || null, tab,
    })
    await sendPushToMember({ memberId: recipientId, title, body, tab })
  } catch {}
}
...
function appUrl(origin) {
  return origin || process.env.NEXT_PUBLIC_APP_URL || 'https://lyftai.io'
}
...
export async function notifyAssignment({ workspaceId, task, origin }) {
  try {
    ...
    await addNotification({ workspaceId, recipientId: task.assignee_id, type: 'assignment', title: `Nuovo task: ${task.title}`, body: task.due_date ? `Scadenza ${task.due_date}` : '', taskId: task.id })
  } catch {}
}
...
export async function notifyStatusChange({ workspaceId, task, origin }) {
  try {
    ...
    await addNotification({ workspaceId, recipientId: task.assignee_id, type: 'status', title: `${task.title} → ${label}`, taskId: task.id })
  } catch {}
}
