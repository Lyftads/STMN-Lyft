...
export async function ensureProjectChannel(admin, { workspaceId, projectId, projectName, createdBy, memberIds = [] }) {
  ...
  // Se un canale con lo stesso nome esiste GIÀ e non è legato a nessun
  // progetto, lo si adotta invece di creare un gemello: il gruppo che il team
  // usa da settimane è quello, e ritrovarsi "promo-autunno" e
  // "promo-autunno-2" affiancati è il modo più rapido per spezzare in due una
  // conversazione.
  try {
    const { data: orphan } = await admin.from('channels').select('id, project_id')
      .eq('workspace_id', workspaceId).eq('name', base).is('project_id', null).maybeSingle()
    if (orphan) {
      await admin.from('channels').update({ project_id: projectId }).eq('id', orphan.id)
      await syncChannelMembers(admin, { channelId: orphan.id, memberIds })
      return { channelId: orphan.id, adopted: true }
    }
  } catch {}
