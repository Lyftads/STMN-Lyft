...
// ── blocchi HTML ────────────────────────────────────────────────────────────
const wrap = inner => `<div style="font-family:-apple-system,Segoe UI,Roboto,Arial,sans-serif;max-width:680px;margin:0 auto;color:#1d1d1d;background:#f5f5f5;padding:0">${inner}</div>`
...
function kpi(label, value, sub) {
  return `<td style="padding:6px"><div style="background:#faf9ff;border:1px solid #ece9ff;border-radius:12px;padding:12px 10px;text-align:center">
    ...
    <div style="font-size:20px;font-weight:800;color:#1d1d1d;margin-top:3px">${value}</div>
...
function simpleTable(headers, rows) {
  ...
  const th = headers.map(h => `<th style="text-align:left;padding:7px 10px;border-bottom:2px solid #1d1d1d;font-size:12px">${esc(h)}</th>`).join('')
...
function creativeCard(c) {
  ...
  const copy = c.copy ? `<div style="font-size:13px;line-height:1.5;color:#1d1d1d;margin:6px 0"><b>Copy:</b> ${esc(String(c.copy).slice(0, 320))}${String(c.copy).length > 320 ? '…' : ''}</div>` : ''
...
// ── HTML completo ───────────────────────────────────────────────────────────
export function renderReportHTML({ liveData, narrative = {}, tasks = [], brandName = 'il tuo brand', periodLabel = 'ultime 4 settimane', appUrl = 'https://lyftai.io' }) {
  ...
  // FINESTRA UNICA: le ultime 4 settimane COMPLETE (lunedi'→domenica), scelte
  // per DATA su entrambe le serie. Prima si prendevano "le ultime 4 righe" di
  // ciascuna: Shopify finiva al 27/07 mentre Meta arrivava al 03/08, cioe' alla
  // settimana in corso con un giorno solo di dati. Risultato: il MER divideva i
  // ricavi di un periodo per la spesa di un altro, e i quattro numeri del
  // riquadro Advertising erano su finestre diverse fra loro.
  const weekKeys = (() => {
    const d = new Date()
    const dow = (d.getUTCDay() + 6) % 7               // 0 = lunedi'
    d.setUTCDate(d.getUTCDate() - dow - 7)            // lunedi' dell'ultima settimana COMPLETA
    const keys = []
    for (let i = 3; i >= 0; i--) {
      const x = new Date(d); x.setUTCDate(x.getUTCDate() - i * 7)
      keys.push(x.toISOString().slice(0, 10))
    }
    return new Set(keys)
  })()
  const inWindow = (rows) => {
    const sel = rows.filter(r => weekKeys.has(String(r.date || r.key || '').slice(0, 10)))
    return sel.length ? sel : rows.slice(-4)          // serie con chiavi diverse → comportamento precedente
  }
  const last4 = inWindow(wk)
  const ma4 = inWindow(maWk)
  ...
  const adSpend = ma4.reduce((s, w) => s + (Number(w.spend) || 0), 0)
  ...
  // CTR/CPM/Frequency ricalcolati sulla STESSA finestra della spesa: prima
  // venivano dall'ultima riga (una settimana), accanto a una spesa di quattro.
  const adImpr = ma4.reduce((s, w) => s + (Number(w.impressions) || 0), 0)
  const adClicks = ma4.reduce((s, w) => s + (Number(w.linkClicks) || 0), 0)
  const adCtr = adImpr ? (adClicks / adImpr) * 100 : 0
  const adCpm = adImpr ? (adSpend / adImpr) * 1000 : 0
  // La frequency non si somma (e' impression/reach): media pesata sulle impression.
  const adFreq = adImpr
    ? ma4.reduce((s, w) => s + (Number(w.frequency) || 0) * (Number(w.impressions) || 0), 0) / adImpr
    : 0
  ...
  html += card(`${narrative.executive ? `<p style="font-size:15px;line-height:1.6;margin:0 0 14px;color:#1d1d1d">${esc(narrative.executive)}</p>` : ''}
    ${kpiRow([
  ...
  adsInner += kpiRow([
    ...
    kpi('CTR', pct(adCtr)),
    kpi('CPM', eur(adCpm)),
    kpi('Frequency media', dec(adFreq, 2)),
  ])
  ...
  if (tasks && tasks.length) {
    ...
    html += card(`${h2('✅', 'Task della settimana assegnati')}
      <table width="100%" cellpadding="0" cellspacing="0" style="border-collapse:collapse">
        <thead><tr>${['Task', 'Assegnato a', 'Priorità', 'Scadenza', 'Proposto da'].map(h => `<th style="text-align:left;padding:7px 10px;border-bottom:2px solid #1d1d1d;font-size:12px">${h}</th>`).join('')}</tr></thead>
