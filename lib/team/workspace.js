  ...
  if (effectiveId !== userId) {
    try {
      const { data: ag } = await admin
        ...
        .maybeSingle()
      if (ag) {
        // Agency verificata: comanda il workspace cliente. Se pero' ha anche
        // una riga in team_members li' dentro, quel memberId va portato con
        // se': e' la sua identita' in quello spazio. Senza, i messaggi in
        // LyftTalk nascevano con l'autore vuoto e la persona compariva come
        // "Utente" senza volto, pur avendo un profilo con nome e foto.
        try {
          const { data: mine } = await admin
            ...
            .eq('user_id', userId)
            .maybeSingle()
          if (mine?.id) return { ...ownerCtx, memberId: mine.id }
        } catch {}
        return ownerCtx
      }
