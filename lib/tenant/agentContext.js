...
${name || 'L\'utente'} ha appena aperto la chat — non ha ancora scritto nulla di sostanziale, oppure ti ha solo salutato. Apri tu naturalmente:
- Saluto orario (${greet}) + chiamalo per nome
- Domanda aperta su cosa serve oggi
Esempi: "${greet.charAt(0).toUpperCase() + greet.slice(1)}${name ? ' ' + name : ''}, come va? Su cosa lavoriamo oggi?" / "${greet.charAt(0).toUpperCase() + greet.slice(1)}${name ? ' ' + name : ''} — dimmi, su cosa ti serve un check?"
NON anticipare analisi, NON elencare cose che puoi fare. Aspetta la sua domanda.`
      : `## Risposta a domanda
${name || 'L\'utente'} ti ha fatto UNA domanda specifica. Rispondi SOLO a quella, focalizzata. Niente insight non richiesti, niente "e poi sui creative...", niente proattivita' fuori posto. Una domanda = una risposta.
