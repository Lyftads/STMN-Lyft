...
import { recall, recallKnowledge, embedQuery, formatMemoriesForPrompt, formatKnowledgeForPrompt, extractMemoriesFromTurn, extractMemoriesFromData, rememberBatch } from './agentMemory'
...
export async function buildAgentContext({ agentId, query, memoryLimit = 6, conversationLength = 0, includeConversation = true, greetingOnly = false }) {
  ...
  // UN solo embedding della domanda, riusato dai quattro recall qui sotto.
  // Prima ognuno se lo ricalcolava: quattro chiamate identiche all'API per
  // ogni singolo messaggio.
  const queryEmbedding = query ? await embedQuery(query).catch(() => null) : null

  const [brandBlock, memories, autoScanMemories, callMemories, knowledge] = await Promise.all([
    buildBrandContext(),
    userId && query
      ? recall({ userId, agentId, query, limit: memoryLimit, queryEmbedding })
      : Promise.resolve([]),
    userId && query && agentId !== 'auto-scan'
      ? recall({ userId, agentId: 'auto-scan', query, limit: 4, minImportance: 5, queryEmbedding })
      : Promise.resolve([]),
    ...
    userId && query && agentId !== 'team-call'
      ? recall({ userId, agentId: 'team-call', query, limit: 3, minImportance: 6, queryEmbedding })
      : Promise.resolve([]),
    // Knowledge globale (corso/YouTube) — condivisa da TUTTI gli agent/clienti.
    query ? recallKnowledge({ query, limit: 4, queryEmbedding }) : Promise.resolve([]),
  ])
  ...
  // Apertura SOLO se non c'è ancora una domanda vera: con <=1 la PRIMA domanda
  // ("quanto ho speso?") riceveva il ramo "apri tu la conversazione" e restava
  // senza risposta. Il chiamante passa isGreetingOnly quando il primo messaggio
  // è solo un saluto.
  const isFirstTurn = conversationLength === 0 || (conversationLength <= 1 && greetingOnly)
  ...
  if (memBlock) {
    // Precedenza esplicita: il CONTESTO BRAND vince sulle memorie. Una risposta
    // sbagliata diventa una memoria, e la memoria la tiene in vita: il 7 ago il
    // Cervello continuava a dire "lavoro per STMN Fitness" rileggendo le proprie
    // risposte di prima del cambio nome, col prompt ormai gia' pulito.
    parts.push(`## MEMORIE RILEVANTI (apprese da conversazioni precedenti)\n${memBlock}\n\nATTENZIONE: sull'IDENTITA' dell'azienda — nome, cosa vende, settore — fa fede SOLO il CONTESTO BRAND qui sopra. Se una memoria dice un nome o un prodotto diverso, quella memoria e' vecchia: ignorala e non ripeterla.`)
  }
  if (callBlock) {
