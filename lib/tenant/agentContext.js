...
import { recall, formatMemoriesForPrompt, extractMemoriesFromTurn, extractMemoriesFromData, rememberBatch } from './agentMemory'
...
export async function buildAgentContext({ agentId, query, memoryLimit = 6, conversationLength = 0 }) {
  ...
  const [brandBlock, memories, autoScanMemories] = await Promise.all([
    buildBrandContext(),
    userId && query
