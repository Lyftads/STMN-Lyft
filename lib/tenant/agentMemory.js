...
// Embedding della QUERY, calcolato UNA volta per richiesta e riusato dai vari
// recall. Prima ogni recall se lo ricalcolava per conto suo: per un singolo
// messaggio partivano QUATTRO chiamate identiche all'API embeddings (agent,
// auto-scan, call, knowledge) — stesso testo, stesso risultato, quattro volte
// costo e quota. Micro-cache per testo: utile anche fra richieste ravvicinate.
const embedCache = new Map()          // testo → { emb, ts }
const EMBED_CACHE_TTL = 5 * 60_000
export async function embedQuery(text) {
  if (!text) return null
  const key = String(text).slice(0, 2000)
  const hit = embedCache.get(key)
  if (hit && Date.now() - hit.ts < EMBED_CACHE_TTL) return hit.emb
  const emb = await getEmbedding(key)
  if (emb) {
    embedCache.set(key, { emb, ts: Date.now() })
...
// minSim: misurato empiricamente (31 lug) — memorie ON-TOPIC stanno a 0.49-0.68,
// il rumore ("ciao", domande fuori tema) a 0.15-0.31. Senza soglia il limit
// riempiva SEMPRE il contesto con memorie non pertinenti.
export async function recall({ userId, agentId, query, limit = 5, minImportance = 1, minSim = 0.38, queryEmbedding = null }) {
  if (!userId || !agentId || !query) return []

  const admin = getAdminSupabase()
  if (!admin) return []

  const queryEmb = queryEmbedding || await embedQuery(query)
  if (!queryEmb) return []
  ...
  try {
    ...
    let memories = Array.isArray(data) ? data : []
    if (minSim > 0) {
      const sim = m => Number(m.similarity ?? m.sim ?? m.score ?? 1)
      memories = memories.filter(m => !Number.isFinite(sim(m)) || sim(m) >= minSim)
    }
...
// minSim 0.50: a 0.15 QUALSIASI query (anche "ciao") superava la soglia e la
// knowledge veniva sempre iniettata; on-topic reale misurato a 0.59-0.63.
export async function recallKnowledge({ query, limit = 4, minSim = 0.50, queryEmbedding = null }) {
  if (!query) return []
  const admin = getAdminSupabase()
  if (!admin) return []
  const queryEmb = queryEmbedding || await embedQuery(query)
...
export async function extractMemoriesFromTurn({ userMessage, assistantMessage, agentId }) {
  ...
  const extractorPrompt = `Sei un estrattore di memorie per agent AI. OGNI insight deve citare i numeri esatti (valore attuale e confronto), la causa più probabile e l'implicazione operativa (cosa conviene fare o tenere d'occhio) — mai osservazioni generiche. Dato uno scambio user→assistant, identifica fatti DURATURI da ricordare per le prossime conversazioni.

Estrai (sii GENEROSO: è meglio una memoria utile in più che una in meno):
- PREFERENZE esplicite dell'utente ("preferisco risposte concise", "non usare emoji")
- OBIETTIVI e SOGLIE dichiarati ("il MER target è 2.5x", "voglio stare sotto 30€ di CAC")
- FATTI specifici del brand/business ("lanciamo sempre il martedì", "non vendiamo integratori", "il prodotto X è il best seller")
- DECISIONI e INTENZIONI ("da lunedì spengo la campagna Y", "testiamo il nuovo creative Z")
- INTERESSI RICORRENTI dell'utente (quali metriche/tab/canali chiede più spesso: "chiede spesso il CAC dei nuovi clienti")
- INSIGHT non banali emersi nella conversazione, con i numeri che li ancorano

NON estrarre:
- Riepiloghi/parafrasi della risposta appena data
- Saluti, ringraziamenti, follow-up senza contenuto
- Numeri di un periodo specifico che invecchiano subito (es. "ieri ha speso 200€"), a meno che non siano un record o una soglia dichiarata
...
Output JSON: { "insights": [{ "content": "...", "type": "anomaly|trend|milestone", "importance": 5-9 }] }`
  ...
  // Knowledge (corso/video): SOLO per decidere quali variazioni contano davvero
  // e interpretarle — MAI per inventare anomalie non presenti nei dati.
  let kbBlock = ''
  try {
    const kbItems = await recallKnowledge({ query: 'anomalie performance marketing meta ads scaling retention KPI e-commerce', limit: 4 })
    kbBlock = formatKnowledgeForPrompt(kbItems)
  } catch {}
  ...
  try {
    const res = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: sysPrompt },
          ...(kbBlock ? [{ role: 'system', content: `${kbBlock}\n\nUsa questi principi SOLO per stabilire quali variazioni nei dati sono davvero rilevanti e per una breve interpretazione. NON inventare anomalie non presenti nei numeri: ogni insight resta ancorato ai dati esatti e al timeframe.` }] : []),
