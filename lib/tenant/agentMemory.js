...
export async function extractInsightsFromComparison({ last_24h, last_7d, last_30d, userId, date }) {
  const key = process.env.OPENAI_API_KEY
  if (!key || !userId) return []

  // Compatta input: estraiamo solo le metriche rilevanti dai 3 timeframe
  const summary = {
...
Ricevi i KPI degli ultimi 24h, 7gg e 30gg. Devi identificare:
