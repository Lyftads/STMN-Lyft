...
export async function buildBrandContext() {
  ...
  const userName = brand?.userName || (/stmn/i.test(brand?.companyName || '') ? 'Marino' : "l'utente")
  const companyName = brand?.companyName || 'il tuo brand'

  // Brand identity vuota: default STMN-specific SOLO per il workspace STMN.
  // Gli altri tenant ricevono un blocco neutro — mai i fatti di un'altra
  // azienda (fix 31 lug: i clienti senza form compilata ereditavano STMN).
  if (!brand || isEmptyBrand(brand)) {
    if (/stmn/i.test(companyName)) {
      return [
