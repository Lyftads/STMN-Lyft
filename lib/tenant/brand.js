...
//
// - Se brand_identity e' vuota o nulla → blocco neutro (nessun default con i
...
export async function buildBrandContext() {
  ...
  const userName = brand?.userName || "l'utente"
  const companyName = brand?.companyName || 'il tuo brand'

  // Brand identity vuota → blocco neutro. Qui c'era un default con i fatti di
  // STMN (catalogo, brand guard) usato quando il workspace era il suo: STMN non
  // esiste piu' come azienda, e un default con i prodotti di qualcun altro non
  // deve poter riapparire.
  if (!brand || isEmptyBrand(brand)) {
    return [
      `UTENTE: ${userName} (founder)`,
      `BRAND: ${companyName}`,
      `Nota: la Brand Identity di questo workspace non e' ancora compilata — basati SOLO sui dati, non inventare dettagli su prodotti o posizionamento del brand.`,
    ].join('\n')
  }
...
// Permette al brand context di degradare a un blocco neutro
