...
export async function withTenantContext(_reqOrIgnored, fn) {
  ...
  // In questa modalita' le esponiamo SOLO all'owner (LYFT_OWNER_USER_ID) e ai
  // contesti senza utente (cron/background). Un utente loggato che NON e'
  // l'owner riceve creds vuote → vede onboarding, MAI i dati di STMN.
  ...
  if (process.env.LYFT_MULTI_TENANT !== 'true') {
    ...
    if (owner) {
      ...
      // C'e' un utente loggato diverso dall'owner → niente env di STMN.
      if (uid && uid !== owner) creds = emptyCreds()
    }
