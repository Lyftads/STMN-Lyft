  ...
  if (process.env.LYFT_MULTI_TENANT !== 'true') {
    ...
    if (owner) {
      if (!isAuthorizedCron(req)) {
        ...
        // L'owner (STMN) sul PROPRIO workspace vede le env. Ogni ALTRO utente
        // loggato: risolviamo le SUE creds per-tenant (companies row + Nango) →
        // i nuovi clienti vedono i propri dati, SENZA mai esporre le env di STMN
        // (gate isOwner = is_beta in getTenantCreds). Anonimo → uid null → vuoto.
        //
        // ECCEZIONE: l'owner che ha SWITCHATO a un workspace agency/cliente
        // (cookie active_workspace ≠ owner) deve vedere QUEL workspace — dati del
        // cliente, vuoti finché non fa l'onboarding — NON i dati di STMN. La
        // risoluzione passa da getTenantCreds → getEffectiveTenantId (che verifica
        // l'autorizzazione via agency_clients, anti-leak).
        let activeWs = null
        try { activeWs = cookies().get('active_workspace')?.value || null } catch {}
        const ownerSwitched = uid === owner && activeWs && activeWs !== owner
        if (uid !== owner || ownerSwitched) creds = await getTenantCreds()
      }
    }
