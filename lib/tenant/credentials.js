  ...
  if (process.env.LYFT_MULTI_TENANT !== 'true') {
    ...
    if (owner) {
      if (!isAuthorizedCron(req)) {
        ...
        // L'owner (STMN) vede le env. Ogni ALTRO utente loggato: risolviamo le
        // SUE creds per-tenant (companies row + Nango) → i nuovi clienti vedono
        // i propri dati, SENZA mai esporre le env di STMN (gate isOwner in
        // getTenantCreds). Anonimo → uid null → company null → tutto null = vuoto.
        // NB: questo NON tocca STMN (owner) né i cron (isAuthorizedCron sopra).
        if (uid !== owner) creds = await getTenantCreds()
      }
    }
