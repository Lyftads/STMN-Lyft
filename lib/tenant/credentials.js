...
async function authorizedWorkspaceIds(agencyUid, { fresh = false } = {}) {
  ...
  try {
    const admin = getAdminSupabase()
    if (admin) {
      const { data } = await admin.from('agency_clients').select('client_user_id').eq('agency_user_id', agencyUid)
      for (const r of (data || [])) ids.add(r.client_user_id)
    }
  } catch {}
  memberCache.set(agencyUid, { ids, at: Date.now() })
  return ids
}
...
export async function getEffectiveTenantId() {
  ...
  if (active && active !== realUid) {
    ...
    if (!ids.has(active)) ids = await authorizedWorkspaceIds(realUid, { fresh: true })
