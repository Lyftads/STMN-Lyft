...
export async function getTenantCreds() {
  ...
  const connId = company?.nango_connection_id || null
  let nangoMeta = null, nangoKlaviyo = null
  if (connId) {
    ;[nangoMeta, nangoKlaviyo] = await Promise.all([
      getNangoToken({ integrationId: NANGO_INTEGRATIONS.meta, connectionId: connId }).catch(() => null),
      getNangoToken({ integrationId: NANGO_INTEGRATIONS.klaviyo, connectionId: connId }).catch(() => null),
    ])
  }
  ...
  return {
    ...
    meta: {
      // Priorita': token Nango (refresh gestito) → token salvato → env.
    ...
    klaviyo: {
      apiKey: pick(nangoKlaviyo, company?.klaviyo_api_key, process.env.KLAVIYO_API_KEY),
    },
  }
}
...
export async function withTenantContext(_reqOrIgnored, fn) {
  // Fast path #1: env-only mode globale (zero Supabase round-trip)
  if (process.env.LYFT_MULTI_TENANT !== 'true') {
    return tenantStore.run(envOnlyCreds(), fn)
  }
  ...
  try {
    ...
    if (!creds) {
      // Timeout o nessun lookup → degrade graceful a env-only.
      // Marino (STMN beta) ha env vars popolate quindi continua a funzionare.
      creds = envOnlyCreds()
    }
  } catch (e) {
